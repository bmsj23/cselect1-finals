...
void computeSchedule() {
  float softMul = pow(difficultyMultipliers[selectedDifficulty], 0.5f);
  travelTimeMs = (int)(BASE_TRAVEL_TIME_MS / softMul);
  unsigned long t = 0;
  ...
  for (int i=0; i<len; i++) {
    ...
    // scale beats with a softened multiplier to speed up songs less aggressively
